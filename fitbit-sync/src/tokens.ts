/**
 * Gestion des tokens OAuth Google Health.
 ...
 *
 * Différence avec Coros : Google exige client_id ET client_secret au refresh (le
 * client "Desktop" a un secret), et ne fait pas de rotation du refresh_token —
 * on garde donc l'ancien si la réponse n'en renvoie pas.
 */
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Refresh token failed: ${res.status} ${res.statusText}\n${body}\n\n` +
        `Si le refresh_token est expiré/révoqué, relance \`npm run bootstrap\` pour ré-autoriser.`,
    );
  }
...
export async function getValidAccessToken(
  ...
  const refreshed = await refreshAccessToken(clientId, clientSecret, current.refresh_token);
